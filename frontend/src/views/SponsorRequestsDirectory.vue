                ...
                <table class="table table-bordered table hover">
                    ...
                    <tbody>
                        <tr v-for="(adrequest, index) in adrequests" :key="adrequest.id">
                            <td>{{ index + 1 }}</td>
                            <td id="recieved" v-if=" adrequest.owner === 'Influencer' ">Recieved</td>
                            ...
                            <td><router-link :to="{ name: 'SponsorViewCampaignDetails', params: { id: adrequest.campaign.id } }"><button class="btn btn-warning">View</button></router-link></td>
...
export default {
    ...
    methods: {
        async getAdRequests() {
            try {
                const response = await axios.get('/sponsor/adrequests_directory');
